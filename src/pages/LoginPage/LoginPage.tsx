...
function LoginPage({ navigation, route }: ScreenProps<'LoginPage'>) {
  ...
  return (
    ...
    >
      <Card style={{ width: Math.min(width - 40, 420), borderRadius: 12 }}>
          ...
          >
            ...
            <Dialog.Content>
              {step === 1 ? (
                <>
                  <Paragraph>
                    휴대폰 번호를 입력하면 인증번호를 전송합니다.
                  </Paragraph>
                  <TextInput
                    label="휴대폰 번호"
                    value={phoneInput}
                    onChangeText={(t: string) => setPhoneInput(formatPhone(t))}
                ...
                <>
                  <Paragraph>전송된 인증번호를 입력해주세요.</Paragraph>
                  <TextInput
                    label="휴대폰 번호"
                    value={phoneInput}
                    disabled
            ...
            <Dialog.Actions>
              <Button onPress={() => setPhoneModalVisible(false)}>취소</Button>
              {step === 1 ? (
                <Button
                  mode="contained"
                  loading={sending}
                  disabled={sending || rawPhone(phoneInput).length < 9}
                  onPress={async () => await sendPhoneCode(phoneInput)}
                >
                  인증번호 전송
                </Button>
              ) : (
                <Button
                  mode="contained"
                  loading={verifying}
                  disabled={verifying || codeInput.trim().length < 4}
                  onPress={async () =>
                    await verifyPhoneCode(phoneInput, codeInput)
                  }
                >
                  인증 완료
                </Button>
        ...
        <Card.Actions style={{ justifyContent: 'center', paddingBottom: 12 }}>
          <Text style={{ color: '#999', fontSize: 12 }}>
            계정 생성 또는 로그인 시 이용약관 및 개인정보처리방침에 동의합니다.
          </Text>
        </Card.Actions>
      </Card>
    </View>
