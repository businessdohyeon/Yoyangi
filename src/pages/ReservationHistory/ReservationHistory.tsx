    ...
    <SafeAreaView edges={['left', 'right', 'bottom']} style={{ flex: 1 }}>
      ...
      <View style={{ flex: 1, backgroundColor: '#f6f6f6', padding: 12 }}>
        {isLoading ? (
          ...
          <FlatList
            data={reservations}
            keyExtractor={(item, idx) => String(item.reservation_id ?? idx)}
            renderItem={({ item }) => {
              ...
              const statusLabel =
                status === 'PENDING'
                  ...
                  : status === 'REJECTED'
                  ? '예약 거절됨'
                  : status === 'CANCLED' || status === 'CANCELED'
                  ? '예약 취소됨'
                  : '알 수 없음';
                    ...
                    onPress: async () => {
                      try {
                        ...
                        await axiosInstance.patch(
                          apis.urls.cancelReservationById(item.reservation_id),
                          {},
                          {
                            headers: {
                              Authorization: `Bearer ${token}`,
                ...
                >
                  <Card.Content>
                    <Text variant="titleMedium">
                      {item.facility_name || '기관명 없음'}
                    </Text>
                    <Text>예약자: {item.user_name}</Text>
                    <Text>예약일: {item.reserved_date}</Text>
                    <View
                      style={{
                        flexDirection: 'row',
                        alignItems: 'center',
                    ...
                    >
                      ...
                      {status === 'PENDING' ? (
                        <Button mode="outlined" onPress={handleCancel} compact>
                          예약 취소
                        </Button>
                      ) : null}
                    </View>
