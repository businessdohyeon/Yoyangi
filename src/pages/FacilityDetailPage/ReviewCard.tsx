  ...
  return (
    ...
    >
      <Card.Title
        ...
        subtitle={formatDate(reviewData.created_at)}
        left={(props) =>
          reviewData.user ? (
            <Avatar.Text {...props} label={(reviewData.user.name || '?')[0]} />
          ) : (
      ...
      <Card.Content>
        ...
        {reviewData.images && reviewData.images.length > 0 ? (
          <View style={styles.imageRow}>
            {reviewData.images
              .slice(0, 3)
              .map((img: string | { uri?: string }, idx: number) => {
                const uri =
                  typeof img === 'string' ? img : img?.uri ?? String(idx);
                const source = { uri };
                return (
                  <TouchableOpacity
                    key={`${idx}-${uri}`}
                  ...
                  reservationId: String(
                    (
                      reviewData as Partial<
                        Record<'reservation_id' | 'reservationId', number>
                      >
                    ).reservation_id ??
                      (
                        reviewData as Partial<
